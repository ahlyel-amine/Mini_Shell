...
/*   Updated: 2023/05/15 15:42:23 by aahlyel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	and_executer(t_cmd *cmd, int infile, int outfile, int is_pipe)
{
	...
	{
		ret = and_part_executer(((t_and *)cmd)->left, infile, outfile, is_pipe);
		if (ret)
			ret = and_part_executer(((t_and *)cmd)->right, infile, outfile, is_pipe);
	}
	return (ret);
}
